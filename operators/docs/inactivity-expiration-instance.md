...
Each `Template` resource defines the `spec.cleanup.stopAfterInactivity` field (StopAfterInactivity), which specifies the maximum period of inactivity allowed. For example:

- If `StopAfterInactivity` is set to `10d` (10 days), CrownLabs will start sending notifications after 10 days without any access (via browser or SSH).
...
By default, new Templates have `spec.cleanup.stopAfterInactivity` set to `never`, meaning inactivity checks are disabled. Instances created from such templates will never be terminated due to inactivity.
...
## Phase 2: Inactivity Destruction After Powered Off (spec.cleanup.deleteAfterInactivity)

A second timeout, `spec.cleanup.deleteAfterInactivity` (previously `DeleteAfterInactivity`), can be specified in the `Template` resource. This applies exclusively to persistent instances left powered off for a specific time.

- If `spec.cleanup.deleteAfterInactivity` is set to `10d`, CrownLabs will track how long the instance has been powered off.
...
CrownLabs includes a feature that automatically deletes Instances after they reach a maximum age (time passed from creation), as defined by `spec.cleanup.deleteAfterCreation` (previously `DeleteAfterCreation`) in the associated Template.
...
- Each `Template` resource defines `spec.cleanup.deleteAfterCreation`, which specifies the maximum age allowed for Instances. For example:
- If `spec.cleanup.deleteAfterCreation` is set to `20d` (20 days), CrownLabs will send an email when the Instance is about to reach 20 days of age.
- When this timer expires, the Instance is automatically deleted without any possibility for the user to stop this action.

By default, new Templates have `spec.cleanup.deleteAfterCreation` set to `never`, meaning expiration-based deletion is disabled. Instances created from such templates do not have any maximum age constraint.
