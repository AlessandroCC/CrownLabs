...
Each `Template` resource defines the `spec.cleanup.stopAfterInactivity` field (InactivityTimeout), which specifies the maximum period of inactivity allowed. For example:
...
A second timeout, `spec.cleanup.deleteAfterInactivity` (previously `DestroyAfterInactivity`), can be specified in the `Template` resource. This applies exclusively to persistent instances left powered off for a specific time.
...
CrownLabs includes a feature that automatically deletes Instances after they reach a maximum age (time passed from creation), as defined by `spec.cleanup.deleteAfterCreation` (previously `DeleteAfter`) in the associated Template.
