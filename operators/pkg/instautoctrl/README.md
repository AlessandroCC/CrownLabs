...
The template introduces a new field called `StopAfterInactivity`, which specifies the period of time during which the Tenant must not access the Instance for it to be considered inactive and eligible for deletion.
...
Next, the controller checks whether the instance is inactive by comparing its last activity timestamp with the **StopAfterInactivity** value specified in the Template.
...
Note: a single query on Prometheus cannot return more than **11000 data points**. In order to cover all the scenarios, a new parameter `queryStep` has been defined in the Helm Chart to modify the query resolution (query step), based on the **StopAfterInactivity** selected.

After this check, the **crownlabs.polito.it/last-activity** is updated with the most recent timestamp.
If the last access is above the max threshold (defined with the `stopAfterInactivity` field in the **Template** resource), the Instance is declared as **inactive** and (if enabled) email notifications start to be sent at regular interval -`inactiveTerminationNotificationInterval` parameter in the Helm chart.
...
- **Templates**: if the `stopAfterInactivity` is set or modified in a template, the associated instances must be reconciled to recalculate the remaining time of the associated instances.
...
This controller verifies whether the instance has exceeded its maximum lifespan, as defined by the **DeleteAfterCreation** field in the associated **Template** resource.
If exceeded, the instance and its related resources are deleted.

### Detailed Behavior

The controller retrieves all the active Instances and fetches the related **Template** resource. Based on the `DeleteAfterCreation` field of the Template, the maximum lifespan of each Instance is determined.
